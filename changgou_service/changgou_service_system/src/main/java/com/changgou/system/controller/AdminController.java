...
public class AdminController implements AdminApi {
    ...
    public Result login(@RequestBody Admin admin) {
        ...
        if(result){
            ...
            // TODO 查询用户所属权限，电商系统后台我们设置权限为 admin
            List<String> roles= new ArrayList<>();
            if (admin.getLoginName().equals("admin")) { // 假设是管理员
                roles.add("admin");
            }
            //基于工具类生成jwt令牌
            String jwt = JwtUtil.createJWT(UUID.randomUUID().toString(), admin.getLoginName(), null,roles);
            info.put("token",jwt);
            return R.T("登录",info);
        }
        return R.F("登录");
